...
cv::Mat buildHough(cv::Mat sobel) {// единственный аргумент - это результат свертки Собелем изначальной картинки
    ...
    for (int y0 = 0; y0 < height; ++y0) {
        for (int x0 = 0; x0 < width; ++x0) {
            ...
            float strength = sobel.at<float>(x0, y0);// TODO считайте его "силу градиента" из картинки sobel
            ...
            for (int theta0 = 0; theta0 < max_theta; ++theta0) {
                ...
                float r0 = x0*cos(2*PI*theta0/max_theta)+y0*sin(2*PI*theta0/max_theta);
                ...
                int i = theta0;
                int j = r0/sqrt(height*height+width*width)*100;
